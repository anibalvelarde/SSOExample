...
Ext.define('SSO.Security.Proxy', {
    ...
    buildUrl: function (request) {
        ...
        if (!(this.config.resourceName != undefined && this.config.resourceName != '')) {
            return SSO.Security.retrieveItem(SSO.Security.ls_gsfServerApiUrl) + request.getProxy().getUrl();
        }
    ...
    doRequest: function (operation, callback, scope) {
        ...
        if (SSO.Security.isAccessTokenValid(access_token)) {
            ...
            SSO.Security.getAccessToken(SSO.Security.retrieveItem(SSO.Security.ls_environment),
                apiName,
                SSO.Security.retrieveItem(SSO.Security.ls_redirectUrl));
        }
    }
});